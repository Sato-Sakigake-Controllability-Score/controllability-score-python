...
def test_aecs_solution_is_near_grid_optimum_n3() -> None:
    ...
    p, info, WList, P = aecs(A, T, solver_options=sol_opt)
...
def test_uniform_under_full_symmetry() -> None:
    ...
    p, info, _, _ = aecs(A, math.inf, solver_options=PGSolverOptions(max_iter=2000, tol=1e-12))
...
def test_permutation_equivariance() -> None:
    ...
    p, _, _, _ = aecs(A, math.inf, solver_options=opt)

    perm = np.array([2, 1, 0, 3], dtype=int)
    Pm = np.eye(4)[perm]
    A2 = Pm @ A @ Pm.T

    p2, _, _, _ = aecs(A2, math.inf, solver_options=opt)
...
def test_small_perturbation_changes_p_only_mildly() -> None:
    ...
    opt = PGSolverOptions(max_iter=2500, tol=1e-12)
    p1, _, _, _ = aecs(A, math.inf, solver_options=opt)
...
def test_wlist_satisfies_lyapunov_residual_infinite_horizon() -> None:
    ...
    p, info, WList, _ = aecs(A, math.inf, w_options=wopt, inf_keep="all", w_output="orig")
    ...
    for i in range(n):
        ...
        Q = ei @ ei.T
        R = A @ Wi + Wi @ A.T + Q
        print("||Wi||=", np.linalg.norm(Wi, "fro"))
        print("||A Wi + Wi A^T||=", np.linalg.norm(A@Wi + Wi@A.T, "fro"))
        print("||A Wi + Wi A^T - (-Ei)||=",
        np.linalg.norm(A@Wi + Wi@A.T - (-Q), "fro"))
